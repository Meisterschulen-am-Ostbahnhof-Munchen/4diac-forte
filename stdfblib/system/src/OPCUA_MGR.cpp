...
namespace forte::iec61499::system {
  ...
  EMGMResponse OPCUA_MGR::defineIEC61499MgmtObjectType(UA_Server *paServer) {
    EMGMResponse eRetVal = EMGMResponse::Ready;
    // Requested NodeId only -- borrows smMgmtType's static storage, so it must not be assigned
    // to mMgmtTypeId directly: on failure UA_Server_addObjectTypeNode() leaves its outNewNodeId
    // argument untouched, and the destructor would otherwise later call UA_NodeId_clear() on
    // this borrowed pointer, an invalid free of static memory. mMgmtTypeId (already
    // NULL-initialized in the constructor) only ever receives the owned copy written to
    // outNewNodeId on success.
  ...
  EMGMResponse OPCUA_MGR::defineIEC61499ResourceObjectType(UA_Server *paServer) {
    EMGMResponse eRetVal = EMGMResponse::Ready;
    // See the identical comment in defineIEC61499MgmtObjectType() above: requestedResourceTypeId
    // borrows smResType's static storage and must stay separate from mResourceTypeId, which
    // (NULL-initialized in the constructor) only ever receives the owned copy written to
    // outNewNodeId on success.
