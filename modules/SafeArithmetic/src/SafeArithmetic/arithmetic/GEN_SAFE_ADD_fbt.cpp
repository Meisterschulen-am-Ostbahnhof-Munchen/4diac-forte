...
namespace forte::SafeArithmetic::arithmetic {
  ...
  void GEN_SAFE_ADD::readInputData(TEventID) {
    for (TPortId i = 0; i < getFBInterfaceSpec().getNumDIs(); ++i) {
      readData(i, mGenDIs[i], mGenDIConns[i]);
    }
  }

  void GEN_SAFE_ADD::writeOutputData(TEventID) {
  ...
  bool GEN_SAFE_ADD::createInterfaceSpec(const char *paConfigString, SFBInterfaceSpec &paInterfaceSpec) {
  ...
  CEventConnection *GEN_SAFE_ADD::getEOConUnchecked(TPortId paEONum) {
    return (paEONum == 0) ? &conn_CNF : nullptr;
  }

  void GEN_SAFE_ADD::createGenInputData() {
    mGenDIs = std::make_unique<CIEC_ANY_MAGNITUDE_VARIANT[]>(getFBInterfaceSpec().getNumDIs());
  }

  CIEC_ANY *GEN_SAFE_ADD::getDI(size_t paDINum) {
    return &mGenDIs[paDINum];
  }

  CIEC_ANY *GEN_SAFE_ADD::getDO(size_t paDONum) {
    switch (paDONum) {
  ...
  CDataConnection *GEN_SAFE_ADD::getDOConUnchecked(const TPortId paDONum) {
    switch (paDONum) {
