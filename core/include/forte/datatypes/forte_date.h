...
namespace forte {
  ...
  class CIEC_DATE final : public CIEC_ANY_DATE {
    ...
    public:
      ...
      constexpr CIEC_DATE(const CIEC_DATE &paValue) : CIEC_ANY_DATE() {
        setValueSimple(paValue);
      }

      constexpr explicit CIEC_DATE(const TValueType paValue) {
        setTUINT64(paValue);
      }

      constexpr ~CIEC_DATE() override = default;

      constexpr CIEC_DATE &operator=(const CIEC_DATE &paValue) {
      ...
      constexpr std::strong_ordering compare(const CIEC_DATE &paOther) const {
        return static_cast<TForteUInt64>(*this) <=> static_cast<TForteUInt64>(paOther);
      }

      constexpr std::strong_ordering operator<=>(const CIEC_DATE &paOther) const {
  ...
  constexpr CIEC_DATE operator""_DATE(unsigned long long int paValue) {
