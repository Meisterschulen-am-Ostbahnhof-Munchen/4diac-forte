...
namespace forte {
  ...
  class CIEC_UDINT final : public CIEC_ANY_UNSIGNED {
    ...
    public:
      ...
      constexpr CIEC_UDINT(const CIEC_UDINT &paValue) : CIEC_ANY_UNSIGNED() {
        setValueSimple(paValue);
      }

      constexpr CIEC_UDINT(const CIEC_UINT &paValue) : CIEC_ANY_UNSIGNED() {
        setValueSimple(paValue);
      }

      constexpr CIEC_UDINT(const CIEC_USINT &paValue) : CIEC_ANY_UNSIGNED() {
        setValueSimple(paValue);
      }

      constexpr explicit CIEC_UDINT(const CIEC_ANY_INT &paValue) : CIEC_ANY_UNSIGNED() {
        setValueSimple(paValue);
      }

      constexpr explicit CIEC_UDINT(const TValueType paValue) {
        setTUINT32(paValue);
      }

      constexpr ~CIEC_UDINT() override = default;

      constexpr CIEC_UDINT &operator=(const CIEC_UDINT &paValue) {
      ...
      constexpr CIEC_UDINT &operator=(const T &paValue) {
        setValueSimple(paValue);
        return *this;
      }
      ...
      constexpr std::strong_ordering compare(const CIEC_UDINT &paOther) const {
        return static_cast<TValueType>(*this) <=> static_cast<TValueType>(paOther);
      }

      constexpr std::strong_ordering operator<=>(const CIEC_UDINT &paOther) const {
        return compare(paOther);
      }

      constexpr bool operator==(const CIEC_UDINT &paOther) const {

        return compare(paOther) == std::strong_ordering::equal;
      }
  };

  constexpr CIEC_UDINT operator""_UDINT(unsigned long long int paValue) {
