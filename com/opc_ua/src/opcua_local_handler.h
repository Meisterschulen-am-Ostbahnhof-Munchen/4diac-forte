...
namespace forte::com_infra::opc_ua {
  ...
  class COPC_UA_Local_Handler : public COPC_UA_HandlerAbstract,
    ...
    private:
       ...
       *  ConnectionManager backing paServerStrings.mWsHostname (opc.ws://),
       *  storing it in mWsConnectionManager.
...
#ifdef FORTE_COM_OPC_UA_WEBSOCKET
      /**
       * WebSocket ConnectionManager backing the server's opc.ws:// endpoint,
       * created via UA_ConnectionManager_new_LWS_WebSocket (requires open62541
       * built with UA_ENABLE_LWS). Owned by the EventLoop of mUaServer once
       * registered -- freed together with it, torn down in run() by
       * UA_Server_delete().
       */
      UA_ConnectionManager *mWsConnectionManager;
#endif // FORTE_COM_OPC_UA_WEBSOCKET
